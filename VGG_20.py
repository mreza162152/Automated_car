...
def main():
    ...
    train_data = train_obj.flow_from_directory(directory="D:\\Visual Exercise\\Python\\VGG_16\\VGG_16\\real_train", target_size=(width, height))
    test_obj = ImageDataGenerator()
    test_data = test_obj.flow_from_directory(directory="D:\\Visual Exercise\\Python\\VGG_16\\VGG_16\\real_test", target_size=(width,height))
    ...
    check_point = ModelCheckpoint("model_1.h5", monitor='val_accuracy', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
    early_stop = EarlyStopping(monitor='accuracy', min_delta=20, verbose=1, mode='auto', patience=10)

    khela = model.fit_generator(steps_per_epoch=100, generator=train_data, validation_data=test_data, validation_steps=10, epochs=10, callbacks=[check_point, early_stop] )

    
    train_loss = khela.history['accuracy']
    test_loss = khela.history['val_accuracy']
